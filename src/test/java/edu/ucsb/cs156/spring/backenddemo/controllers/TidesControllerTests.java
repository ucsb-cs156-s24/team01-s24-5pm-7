...
public class TidesControllerTests {
  ...
  public void test_getEarthquakes() throws Exception {
    ...
    String startDate = "20230401";
    String endDate = "20230402";
    String station = "9411340";
    when(mockTidesQueryService.getJSON(eq(startDate),eq(endDate), eq(station))).thenReturn(fakeJsonResult);

    String url = String.format("/api/tides/get?startDate=%s&endDate=%s&station=%s",startDate,endDate,station);
